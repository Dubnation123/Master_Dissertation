...
plot_to <- as.Date('2019-12-31')
plot_days <- as.data.frame(seq(plot_from, plot_to, by = 'days'))
colnames(plot_days) <- 'Date'
plot_days$t <- 2017 + as.numeric(difftime(plot_days$Date,gtd)/365)

for (i in 1:n){
  Expect_Price <- dplyr::bind_rows(Expect_Price,data.frame(t(FittedPrice(plot_days,result_Par[i,]))))
}
...
ticker_plot <- subset(ticker,ticker$Date >= as.Date('2018-12-01'))
