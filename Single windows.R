...
from <- as.Date("2017-09-01")
to <- as.Date("2017-12-01")

#Plot Setup
fromplot <- as.Date ("2016-06-16")
toplot <- as.Date("2018-01-31")
...
tryParams <- function (m, omega, tc) {  
  ...
  generate_vector = seq(min(rTicker$t), tc-1/365, 1/365)
...
test <- cma_es(c(0.01, 5, max(rTicker$t)+1/365), residual_obj, lower=c(0.01, 5, max(rTicker$t)+1/365), upper=c(1, 16, max(rTicker$t)+125/365), control=vec_control)
